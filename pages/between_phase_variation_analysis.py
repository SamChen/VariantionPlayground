...
if __name__ == "__main__":
    ...
    predefined_configurations = {
        "mimic pTau":{
            ...
            "default_gt_between_subj_var1": 0.232,
            "default_gt_within_subj_var_left1":  0.232 * 0.1,
            "default_gt_within_subj_var_right1": 0.232 * 0.5,

            "default_gt_between_subj_mean2": 0.34,
            "default_gt_between_subj_var2": 0.120,
            "default_gt_within_subj_var_left2":  0.120 * 0.1,
            "default_gt_within_subj_var_right2": 0.120 * 0.5,
        },
    }
    with st.sidebar:
        with st.form("Predefined Configurations:"):
            ...
            fn_var = st.text_input("Formular for var: ", value="1/n * (2*sigma / m + delta)")
    ...
    for m in range(2,12):
        for seed in range(0, total_trials):
            ...
            df = pd.merge(group1, group2, on="subid",
                          suffixes=("_1", "_2"))

            mean_crossphase_diff = (df.value_2 - df.value_1).mean()
            # var_crossphase_diff = (df.value_2 - df.value_1).var()
            delta = (df.value_2 - df.value_1).var()

            # within_subj_var = pd.concat([group1, group2]).groupby(["subid", "groupid"]).var().value.mean()
            sigma = pd.concat([group1, group2]).groupby(["subid", "groupid"]).var().value.mean()
            # delta  = 1/n * (2*within_subj_var / m + var_crossphase_diff)
            # delta  = (2*within_subj_var / m + var_crossphase_diff)
            var  = eval(fn_var)

            pvalue = tttest_1samp_from_stats(mean_crossphase_diff, var,
                                             n, popmean=0.0).pvalue
