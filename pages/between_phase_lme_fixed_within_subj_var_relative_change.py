...
def run(cfg, sample_size, m, seed):
    ...
    ## TODO: how to use slop in parallel design
    assert cfg["with_dependency"]
    if cfg["with_dependency"]:
        group2, _ = simulation.stats_synthesize_dep(
            sampled_between_subj_means,
            cfg["gt_slop_mean"], cfg["gt_slop_var"],
...
if __name__ == "__main__":
    ...
    with st.form("Configuration:"):
        ...
        col2.write("slop: we estimate phase2's stats based on the slop of linear regression and Phase1")
        # n_subj2                  = col2.number_input("Total number of subjets for each Phase (Phase2): ", value=10)
        gt_slop_mean    = col2.number_input("Ground truth slop mean: ",
                                                     value=1.0,format="%.5f")
        gt_slop_var     = col2.number_input("Ground truth slop var: ",
                                                     value=1.0,format="%.5f")
        gt_within_subj_var_pct2  = col2.number_input("How to deal with the within-subject variance: ",
                                                     value=1.0,format="%.2f")
        st.form_submit_button("Submit")

    gt_within_subj_var_value1 = gt_between_subj_var1 * gt_within_subj_var_pct1 / 100
    gt_within_subj_var_value2 = gt_slop_var          * gt_within_subj_var_pct2 / 100
    ...
    cfg["gt_slop_mean"]    = gt_slop_mean
    cfg["gt_slop_var"]     = gt_slop_var
