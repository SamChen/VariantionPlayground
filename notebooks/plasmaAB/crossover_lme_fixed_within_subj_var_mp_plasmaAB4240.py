...
def run(cfg, sample_size, m_group1, m_group2, seed):
    ...
    if cfg["with_dependency"]:
        group2, _ = simulation.stats_synthesize_dep(
            ...
            (cfg["gt_between_subj_mean2"] - cfg["gt_between_subj_mean1"])*cfg["efficacy"],
...
if __name__ == "__main__":
    ...
    with_dependency        = False # keep it False
    efficacy               = 1.0
    ...
    for ratio in ratios:
        ...
        # config for plasmaAB 42/40
        cfg["gt_between_subj_mean1"]    = 0.09640
        cfg["gt_between_subj_var1"]     = 0.00011
        gt_within_subj_var_pct1         = ratio
        cfg["gt_between_subj_mean2"]    = 0.09640 + (-0.00052*0.5)
        cfg["gt_between_subj_var2"]     = 0.00011
        ...
        for sample_size in sample_sizes:
            for seed in range(0, total_trials):
                ...
                configs.append((cfg, sample_size, 3, 4, seed))
        ...
        df_stats.to_csv(os.path.join(output_folder, f"statistic_estimation_crossover_plasmaAB4240_{ratio}.csv"))
